...
|---|-------|-------------|-----------|
| ~~C1~~ | ~~EN home mobile LCP **7.8s** — render-blocking `header.BgfblYv3.css` (480ms)~~ ✅ **PARTIAL FIX 2026-04-25 (Batch G2)** — header CSS inlined into Base.astro `<head>`, `header.*.css` no longer emitted (8.8 KB shaved across all 122 pages). Remaining: Cloudflare Email Obfuscation (482ms) — disable in CF Pages dashboard → Settings → Functions → "Email Obfuscation: Off". Font preload was already shipped (commit 095da3d). | `src/components/Header.astro` + `src/layouts/Base.astro` | 05-performance §1 |
| ~~C2~~ | ~~UA pillar mobile LCP **5.5s** — same root cause as C1~~ ✅ **PARTIAL FIX 2026-04-25 (Batch G2)** — same fix as C1. Page-specific `shcho-take-fulfilment.*.css` (17 KB) remains as the next render-blocking layer; can be addressed in a future pass via `is:inline` on the page's own `<style>`. | `src/pages/ua/shcho-take-fulfilment.astro` | 05-performance §1 |
