...
- ~~L3: Per-language sitemap split (currently single `sitemap-0.xml`)~~ ✅ **DONE 2026-04-25 (Batch L)** — new `integrations/sitemap-lang-split.mjs` runs after `@astrojs/sitemap` and before `image-sitemap`, splitting `sitemap-0.xml` into 3: `sitemap-uk.xml` (29 URLs — UA root + /ua/* legacy), `sitemap-ru.xml` (22), `sitemap-en.xml` (57). Aggregate `sitemap-0.xml` is removed. `sitemap-index.xml` references all 3 + `sitemap-images.xml` with per-child `<lastmod>` (L5). `_headers` updated with explicit Content-Type for each new file (M7 pattern). All 3 lang sitemaps validated well-formed via Python ElementTree. Submit each to GSC International Targeting for per-locale Coverage signals.
